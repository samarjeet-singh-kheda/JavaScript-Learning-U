...
dogs.forEach((dogInfo) => {
  dogInfo.recFood = Math.trunc(dogInfo.weight ** 0.75 * 28);
});
console.log(dogs);
...
const ownersEatTooMuch = dogs
  .flatMap(({ owners, curFood, recFood }) =>
    curFood > recFood ? owners : null
  )
  .filter((owner) => owner);
...
const ownersEatTooLittle = dogs
  ...
  // )
  // .filter((owner) => owner);
  .filter(({ curFood, recFood }) => curFood < recFood)
  .flatMap(({ owners }) => owners);
console.log(ownersEatTooLittle);

console.log(
  // ownersEatTooMuch.reduce(
  //   (str, owner, idx) => `${owner}'s${idx ? " and" : ""} ` + str,
  //   "dogs eat too much!"
  // )
...
console.log(dogs.some(({ curFood, recFood }) => curFood === recFood));

console.log(
  dogs.some(
    ({ curFood, recFood }) => curFood < 1.1 * recFood && curFood > 0.9 * recFood
  )
);
...
console.log(
  dogs
    .filter(
      ({ curFood, recFood }) =>
        curFood < 1.1 * recFood && curFood > 0.9 * recFood
    )
    .flatMap(({ owners }) => owners)
);

const dogSorted = dogs.sort((a, b) => a.recFood - b.recFood);
